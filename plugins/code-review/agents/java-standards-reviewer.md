...
description: 基于严格 diff 范围对 Java 变更行进行代码规范审查
...
3. 若候选文件或变更行为空，直接返回 `[]`
4. 使用 Read 工具读取 `${CLAUDE_PLUGIN_ROOT}/skills/java-code-review/references/java-rules.md`，获取完整的规则定义
...
- `ruleId` 必须与 `java-rules.md` 中的编号完全一致
- 无问题时返回空数组 `[]`
