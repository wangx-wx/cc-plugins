...
1. 读取领域的 include/exclude 范围、关键词、负责人、项目文档和语雀来源，并记录触发时的 Git `HEAD`。
...
5. 子代理报告领域背景、业务术语、观察到的行为、可能的规则与不变量、能力、边界、跨域关系、少量有效代码锚点，以及反直觉、高风险或无法解释的设计。子代理不得决定领域语义，也不得直接修改 L1、ADR 或 `.review`。
...
7. 使用 `assets/L1.md` 生成或更新 `docs/kb/L1/<domain_id>.md`。新文件使用 `status: draft`。
8. 流程脚本将代码基线、范围快照、语雀候选和 archive 引用写入 `docs/kb/.meta/L1/<domain_id>.json`；领域解释和必要证据写入 L1，Agent 不手动维护机器元数据。
9. 按 [领域澄清与 ADR](domain-questioning.md) 处理需要人工解释的术语、规则、边界和设计原因。

## L1 内容边界

L1 解释业务，不罗列完整类、方法、URL、DTO、枚举、表或消息主题。机器清单放入 `.meta`，L1 只保留理解能力或核验规则所需的少量锚点。

“领域上下文”说明该领域为何存在、服务哪些角色或业务阶段、核心对象如何关联，以及与外部系统或其他领域的上下文关系。代码和文档无法说明的背景只能在人工确认后写入。

术语与规则必须分开：

- “术语”回答一个领域词汇准确指什么，使用按术语分节的自然语言定义，可注明应避免的别名和概念边界。
