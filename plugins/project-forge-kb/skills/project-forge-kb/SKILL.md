...
description: 从人工划定的范围构建或增量更新项目知识库：扫描代码与归档资料，经人工访谈生成可审核的 L1 业务模块知识、ADR 和 L0 索引。当用户提到给某个仓库或项目建知识库、生成或更新 docs/kb、业务模块知识、L0/L1 索引、把项目文档或语雀文档归档进知识库，或在代码变更后做知识库增量更新时，都应使用本 Skill。用户沿用旧称（领域知识库、领域知识、领域术语、领域规则、单仓知识库）时同样适用；只做普通文档编辑或与知识库无关的代码改动时不要使用。
---
